				...
				<?php
					...
					if(isset($_GET['zoekTerm']) || isset($_GET['rubriek'])){
						if(isset($_GET['zoekTerm'])){
							...
							$secondWhere=" AND V.Titel LIKE '%".$zoekTerm."%' ";
						}
						if(isset($_GET['Rubrieknummer'])){
							$rubriek=$_GET['Rubrieknummer'];
							if($rubriek>0){
								$thirdWhere=" AND R.RubriekOpLaagsteNiveau=".$rubriek;
							}
						}
					}
					if (isset($_GET["page"])) {
					...
					$sql = "SELECT * FROM ( 
									SELECT V.Voorwerpnummer, V.Titel, V.Beschrijving, V.LooptijdeindeDag, V.LooptijdEindeTijdstip, ROW_NUMBER() OVER (ORDER BY V.Voorwerpnummer) as row FROM Voorwerp V INNER JOIN VoorwerpInRubriek R ON V.Voorwerpnummer = R.Voorwerp WHERE VeilingGesloten='Nee'".$secondWhere." ".$thirdWhere."
					...
					$sqlTellen = "SELECT Voorwerpnummer FROM Voorwerp  V INNER JOIN VoorwerpInRubriek R ON V.Voorwerpnummer = R.Voorwerp WHERE VeilingGesloten='Nee'".$secondWhere." ".$thirdWhere."";
...
$c->setSecond($eindTijdSeconde);
			...
			<?php					
					$aantalGetekend=0;
					echo "pagina's: ";
					for ($i=$page-5; $i<=$total_pages; $i++) {
						if($i<1){
							$i=1;
						}
						$aantalGetekend++;
						if($aantalGetekend<9){
							echo "<a href='productpagina.php?page=".$i."&zoekTerm=".$zoekTerm."&Rubrieknummer=".$rubriek."'>"; if($i==$page){ echo '<b>'.$i.'</b>'; } else{ echo $i; } echo " </a> ";
						}
					}
					echo "...";
					echo "<a href='productpagina.php?page=".$total_pages."&zoekTerm=".$zoekTerm."&Rubrieknummer=".$rubriek."'>"; echo " ".$total_pages; echo " </a> ";
			?>
		</div>
		&nbsp;
	</div>
